...
public class TC_AM_EnterVehicleData_006 extends BaseClass {
	...
	public void TC_AM_EnterVehicleData_006() {
		...
		Assert.assertEquals(enterVehicleData.getMsgOfMakeDropdown().getText(), expectedData,"Make dropdown error message is mismatched");
		explicitwait.until(ExpectedConditions.visibilityOf(enterVehicleData.getIconmMakeDropdown()));
		Assert.assertEquals(enterVehicleData.getIconmMakeDropdown().getCssValue("background-position"),DataReader.fromPropertyFile("asteriskPosition"),"Negative data"+makeOptions.get(1).getText() +"is not acepted");
	}
}
